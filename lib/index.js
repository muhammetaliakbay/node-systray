...
var getWindowsBinaryName = function () {
    if (os.arch() === 'ia32') {
...
var getBinaryPath = function (debug) {
    ...
    var binExists = fs.existsSync(binPath);
    if (binExists) {
        return binPath;
    }
    var localBinPath = path.resolve(process.cwd(), binName);
    var localBinExists = fs.existsSync(localBinPath);
    if (localBinExists) {
        return localBinPath;
    }
...
var getTrayBinPath = function (debug, copyDir) {
    ...
    var binPath = getBinaryPath(debug);
    var binName = ({
        win32: getWindowsBinaryName(),
        darwin: "tray_darwin" + (debug ? '' : '_release'),
        linux: "tray_linux" + (debug ? '' : '_release'),
